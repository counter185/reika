...
$PathReleaseInfoLinux = "reika.Linux/ReleaseInfo.cs"
...
$textNow = [System.IO.File]::ReadAllText($PathReleaseInfoLinux)
$textNow = $textNow -replace '"[^"]*"\/\*HINT_VERSION_INFO\*\/', ('"'+ $env:VERSION_NAME +'"')
$textNow = $textNow -replace '"[^"]*"\/\*HINT_GIT_REF\*\/', ('"'+ $env:VERSION_REF +'"')
Write-Output "Writing: $textNow"
[System.IO.File]::WriteAllText($PathReleaseInfoLinux, $textNow)